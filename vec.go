...
func (v Vector2) Wrap(lens Vector2) Vector2 {
	return Vec2(m.Wrap(v.X, lens.X), m.Wrap(v.Y, lens.Y))
}

// Wrap `len` to each `v` element.
func (v Vector2) Wrap1(len float64) Vector2 {
	return Vec2(m.Wrap(v.X, len), m.Wrap(v.Y, len))
}

// Make `v` elements absolute.
func (v Vector2) Abs() Vector2 {
	return Vec2(m.Abs(v.X), m.Abs(v.Y))
}
...
func (v Vector2) Floor() Vector2 {
	return Vec2(m.Floor(v.X), m.Floor(v.Y))
}

// Round `v` elements.
func (v Vector2) Round() Vector2 {
	return Vec2(m.Round(v.X), m.Round(v.Y))
}
...
func (v Vector2) MoveTowards(other Vector2, dlt float64) Vector2 {
	return other.Sub(v).MagSet(m.Min(dlt, v.Dst(other))).Add(v)
}
