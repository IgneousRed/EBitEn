...
func DrawRectangle(scr *Image, pos, size Vector2, clr Color) {
	DrawTriangles(scr, Trigs{[]Vector2{
		pos,
		pos.Add(Vec2(size.X, 0)),
		pos.Add(Vec2(0, size.Y)),
		pos.Add(Vec2(size.X, size.Y)),
	}, []uint16{0, 1, 2, 1, 2, 3}}, clr)
}
func DrawLine(scr *Image, a, b Vector2, thickness float64, clr Color) {
...
func DrawCircle(scr *Image, pos Vector2, size float64, points int, clr Color) {
	verts := make([]Vector2, points)
	for i := range verts {
